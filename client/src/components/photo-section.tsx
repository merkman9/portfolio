...
function OptimizedImage({ src, alt, className, onClick, onKeyDown, tabIndex, isChanging }: OptimizedImageProps) {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 rounded transition-all duration-500">
          <div className="flex flex-col items-center gap-3">
            <div className="relative">
              <div className="animate-pulse h-8 w-8 bg-charcoal/20 rounded-full"></div>
              <div className="absolute inset-0 animate-ping h-8 w-8 bg-charcoal/10 rounded-full"></div>
            </div>
      ...
      {/* Error state */}
      {hasError && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-50 rounded text-gray-400 transition-all duration-300">
        ...
        <img
          ...
          className={`${className} transition-all duration-700 ease-out transform ${
            isLoading 
              ? 'opacity-0 scale-95' 
              : imageLoaded 
                ? `opacity-100 scale-100 ${isChanging ? 'brightness-75' : 'brightness-100'}` 
                : 'opacity-60 scale-98'
          }`}
          onClick={onClick}
          onKeyDown={onKeyDown}
...
export default function PhotoSection() {
  ...
  const nextFilmImage = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => setFilmImageChanging(false), 100);
    }, 100);
  };
  ...
  useEffect(() => {
    ...
    // Preload next and previous SEM images
    const nextSemIndex = (currentSemIndex + 1) % semImages.length;
    const prevSemIndex = (currentSemIndex - 1 + semImages.length) % semImages.length;
    preloadImage(semImages[nextSemIndex].src);
    preloadImage(semImages[prevSemIndex].src);

    // Preload next and previous film images
    const nextFilmIndex = (currentFilmIndex + 1) % filmImages.length;
    const prevFilmIndex = (currentFilmIndex - 1 + filmImages.length) % filmImages.length;
    preloadImage(filmImages[nextFilmIndex].src);
    preloadImage(filmImages[prevFilmIndex].src);
  }, [currentSemIndex, currentFilmIndex, semImages, filmImages]);
